...
def runAndCollect(binaryPath: str, instancesPath: str, outputPath: str,  validatorPath: str, timeout: int, additional_params: str, runwatch_path: str, binaries_path):
    global get_figure
    global figures_all
    ...
    for instancedir in [dir for dir in os.listdir(instancesPath) if os.path.isdir(f"{instancesPath}/{dir}")]:
        ...
        for file in os.listdir(domain_path):
            if not "domain" in file and file.endswith(".hddl"):
                ...
                runwatch_commands.append(f"{num_job} {binaryPath} {output_path}/runwatch_log/{num_job} {num_job} {domain_file_path} {instance_file_path} {binaries_path}\n")
...
if __name__ == "__main__":
    ...
    binaries_path = str(sys.argv[10])
    runAndCollect(binary, instance_path, output_path, validator_path, timeout, additional_params, runwatch_path, binaries_path)
