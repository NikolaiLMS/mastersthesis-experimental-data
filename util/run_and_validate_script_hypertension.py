...
def hasSolution(outputpath: str) -> bool:
    return (not bool(os.system(f"grep -r '\-\-\-\-\-\-\-Plan\-\-\-\-\-\-\-\-\-\-\-' {outputpath}"))) and (not bool(os.system(f"grep -r '<==' {outputpath}")))
    ...
    for instancedir in [dir for dir in os.listdir(instancesPath) if os.path.isdir(f"{instancesPath}/{dir}")]:
        ...
        for file in os.listdir(domain_path):
            if not "domain" in file and file.endswith(".hddl"):
                ...
                runwatch_commands.append(f"{num_job} ruby {binaryPath} {domain_file_path} {instance_file_path} run\n")
